...
class AirlineSchedulingMAEnvironment:
    ...
    def _setup_spaces(self):
        ...
        # Observation spaces - rich state information for each agent
        obs_dim_base = (
                len(self.available_flights) * 8 +  # Flight features
                len(self.available_aircraft) * 4 +  # Aircraft status
                len(self.available_crew) * 3 +  # Crew status
                10  # Global metrics
        )

        obs_dim_fleet = (
                len(self.available_aircraft) * 6 +  # Aircraft details
                len(self.available_flights) * 5 +  # Flight requirements
                8  # Fleet metrics
        )

        obs_dim_crew = (
                len(self.available_crew) * 5 +  # Crew details
                len(self.available_flights) * 4 +  # Flight crew requirements
                6  # Crew metrics
        )

        obs_dim_emissions = (
                len(self.available_aircraft) * 3 +  # Aircraft efficiency
                len(self.available_flights) * 3 +  # Flight environmental impact
                5  # Emissions metrics
        )
    ...
    def reset(self, seed: Optional[int] = None) -> Tuple[Dict[str, np.ndarray], Dict]:
        ...
        # Sample subset of flights for this episode
        episode_flights = np.random.choice(
            self.available_flights,
            size=min(self.max_flights_per_episode, len(self.available_flights)),
            replace=False
        )
        self.current_episode_flights = list(episode_flights)
    ...
    def _process_complex_actions(self, actions: Dict[str, np.ndarray]):
        ...
        for i, should_schedule in enumerate(base_action):
            if should_schedule and len(self.current_episode_flights) > i:
    ...
    def _get_observations(self) -> Dict[str, np.ndarray]:
        ...
        for flight_id in self.current_episode_flights:
            if flight_id in self.flights_lookup:
                flight = self.flights_lookup[flight_id]
                obs_base.extend([
        ...
        # Pad if necessary
        while len(obs_base) < len(self.current_episode_flights) * 8:
            obs_base.extend([0] * 8)
        ...
        # Global metrics
        obs_base.extend([
            len(self.scheduled_flights) / len(self.current_episode_flights),
            ...
            len(set(self.aircraft_assignments.values())) / len(self.available_aircraft),
            len(set([c for crew_list in self.crew_assignments.values() for c in crew_list])) / len(self.available_crew)
        ])

        observations['base_agent'] = np.array(obs_base, dtype=np.float32)

        # Fleet Agent Observation (simplified for brevity)
        obs_fleet = []
        ...
        # Crew Agent Observation (simplified)
        obs_crew = []
        ...
        # Emissions Agent Observation (simplified)
        obs_emissions = []
    ...
    def _get_info(self) -> Dict:
        ...
        try:
            ...
            # Calculate emissions if we have assignments
            current_emissions = 0
            if self.aircraft_assignments:
                current_emissions = self.reward_system._calculate_actual_emissions(
            ...
            return {
                ...
                'emissions_ratio': current_emissions / self.max_emissions if self.max_emissions > 0 else 0,
...
if __name__ == "__main__":
    print("Testing different action space configurations...")
    ...
    # Test 2: Simple action spaces (for basic MARL algorithms)
    print("\n2. Testing simple action spaces:")
    env_simple = make_airline_env(
        flights_csv="flights_data.csv",
        aircraft_csv="aircraft_data.csv",
        crew_csv="crew_data.csv",
        action_space_type="simple",
        max_flights_per_episode=10,
        episode_length=30
    )

    print(f"Simple action space types: {[type(space).__name__ for space in env_simple.action_spaces.values()]}")

    # Test 3: Box action spaces (for continuous control)
    print("\n3. Testing continuous action spaces:")
    env_box = make_airline_env(
        flights_csv="flights_data.csv",
        aircraft_csv="aircraft_data.csv",
        crew_csv="crew_data.csv",
    ...
    print(f"Box action space types: {[type(space).__name__ for space in env_box.action_spaces.values()]}")

    print("\n✅ All action space configurations tested successfully!")
    print("\nRecommendations:")
    print("- Use 'discrete' for most MARL algorithms (MAPPO, QMIX, etc.)")
    print("- Use 'simple' for basic algorithms that need single discrete actions")
    print("- Use 'box' for continuous control algorithms (TD3, SAC, etc.)")
