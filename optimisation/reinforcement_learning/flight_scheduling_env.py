...
class FlightSchedulingEnv(gym.Env):
    def __init__(self, data, emission_limit=1000, budget_cap=50000):
        ...
        self.action_space = spaces.MultiDiscrete(
            [2, self.num_aircraft, self.num_pilots, self.num_crew] * self.num_flights
        )
    ...
    def step(self, action):
        ...
        action = np.array(action).reshape(self.num_flights, 4)
        ...
        for i, (w_i, a_idx, p_idx, c_idx) in enumerate(action):
            if w_i == 1:
                scheduled_flags[i] = 1
                revenue += Ri[i] * (1 + a * Pi[i])
                total_cost += Cj[a_idx] * Di[i]  * phi[i]  # weather-adjusted aircraft cost
                total_cost += Sm_p[p_idx] * Di[i] # pilot salary
                total_cost += Sn_c[c_idx] * Di[i] # crew salary

                pilot_hours[p_idx] += Di[i]
                crew_hours[c_idx] += Di[i]
        ...
        penalty_pilot = ((beta+1)*largest_pilot_hours_over) / (beta *largest_crew_hours_over + 1)
        ...
        pilot_hour_state = np.clip((Hp - pilot_hours) / Hp, 0, 1)  # normalized left hours
        crew_hour_state = np.clip((Hc - crew_hours) / Hc, 0, 1)  # normalized left hours
